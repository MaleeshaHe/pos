            ...
            <div className="space-y-2">
              {items.map((item, index) => {
                ...
                return (
                  <div
                    key={item.productId}
                    className="bg-white rounded-lg p-3 border-2 border-gray-200 hover:border-blue-300 transition-all shadow-sm hover:shadow group"
                    style={{
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span className="text-gray-600 font-medium">{formatCurrency(item.unitPrice)} each</span>
                    ...
                    <div className="flex items-center gap-2 mb-2 py-2 px-3 bg-gray-50 rounded-lg border border-gray-200">
                      <span className="text-xs font-semibold text-gray-700">Quantity:</span>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleQuantityChange(item.productId, -1)}
                          className="w-8 h-8 flex items-center justify-center bg-white hover:bg-gray-100 text-gray-700 rounded border border-gray-300 font-bold transition-all active:scale-95"
                        >
                          <Minus size={16} />
                        </button>
                        ...
                        <input
                          ...
                          className="w-16 h-8 text-center text-sm font-bold text-gray-900 border-2 border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white"
                        />

                        <button
                          onClick={() => handleQuantityChange(item.productId, 1)}
                          disabled={item.quantity >= item.currentStock}
                          className={`w-8 h-8 flex items-center justify-center rounded border font-bold transition-all active:scale-95 ${
                      ...
                      <span className="text-xs text-gray-500 ml-auto">
                        {item.quantity} × {formatCurrency(item.unitPrice)} = <span className="font-semibold text-gray-700">{formatCurrency(item.quantity * item.unitPrice)}</span>
                      </span>
                    </div>

                    {/* Row 3: Discount */}
                    <div className="flex items-center gap-2 py-2 px-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                      <Percent size={14} className="text-green-600" />
                      <span className="text-xs font-semibold text-green-700">Discount:</span>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
